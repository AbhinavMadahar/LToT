...
% --- Algorithms: prefer algpseudocode (algorithmicx) without conflicts ---
% ICML kit may load 'algorithm' + 'algorithmic'. We neutralize the legacy
% 'algorithmic' and then load 'algpseudocode' so \State, \If, \While, ... work.
\makeatletter
...
% Float environment (safe either way)
\usepackage{algorithm}

% Pseudocode macros used in this manuscript
\usepackage{algpseudocode}  % <-- spelling: algpseudocode (not "algpsuedocode")
...
\usepackage{url}
\usepackage{placeins}
\usepackage{caption}  % control spacing in captions
% \captionsetup[table]{aboveskip=3pt,belowskip=2pt,skip=2pt}  % keep table footprint tight
\usepackage{hyperref}

% Uncomment for camera-ready; keep commented for anonymous re% view.
% % \iclrfinalcopy
%
...
% (removed ICLR-specific tight float spacing to comply with ICML format)
% ------------------------------------------------------------------

% ICML running title
\icmltitlerunning{Lateral Tree-of-Thoughts Surpasses ToT by Incorporating…}
\setlength{\emergencystretch}{3em}
\begin{document}
% \raggedbottom  % reduce large bottom whitespace from page equalization (disabled for ICML)


% == ICML front matter ==
\twocolumn[
\icmltitle{Lateral~Tree\mbox{-}of\mbox{-}Thoughts Surpasses ToT by Incorporating Logically\mbox{-}Consistent, Low\mbox{-}Utility Candidates}

% It is okay to include author info: it will be hidden unless [accepted] is used.
...
\bibliography{icml-submission}  % matches paper/iclr-submission.bib

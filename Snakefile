...
rule all:
    input: WHITE_PAPER_DESTINATION_FILE_PATH,
           ICLR_SUBMISSION_DESTINATION_FILE_PATH


rule iclr_submission:
    output:
        ICLR_SUBMISSION_DESTINATION_FILE_PATH
    input:
        tex = ICLR_SUBMISSION_TEX_FILE_PATH,
        bib = ICLR_SUBMISSION_BIB_FILE_PATH
    log:
        ICLR_SUBMISSION_LOG_FILE_PATH
    shell:
        r"""
        set -euo pipefail
